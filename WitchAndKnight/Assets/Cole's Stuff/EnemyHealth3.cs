...
public class EnemyHealth3 : MonoBehaviour {
	...
	void Start () {
		...
		EnemyHealthText = GameObject.Find("EnemyHealthText3").GetComponent<Text>();
		
		// Set the HUD to display the current amount of health
		EnemyHealthText.text = "" + currentHP + " Enemy 3 HP";
	...
	void OnTriggerStay (Collider collider)
	{
		...
		if (Input.GetButtonDown ("Slash"))
		{
			...
			// Set the HUD to display the current amount of health
			EnemyHealthText.text = "" + Mathf.Round(currentHP) + " Enemy 3 HP";
		}
	}
}
